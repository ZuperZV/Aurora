...
public class TwigItem extends Item {
    ...
    public TwigItem(Item.Properties p_272907_) {
        super(p_272907_);
    }
    ...
    public InteractionResult useOn(UseOnContext p_272607_) {
        ...
        // Check if the block is a TwigableBlock
        if (blockState.getBlock() instanceof TwigableBlock) {
            if (player != null) {
                // Start using the item if the block is TwigableBlock
                ...
                if (blockState.getBlock() instanceof TwigableBlock twigableBlock) {
                    ...
                    if (usedTicks >= 30) {
                        if (!level.isClientSide) {
                            ItemStack skull_twig = new ItemStack(ModItems.SKULL_TWIG.get());
                            player.setItemInHand(player.getUsedItemHand(), skull_twig);
